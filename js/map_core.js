...
function initMap() {
	...
	// Исправление залипания тултипов
	if (map) {
		// Закрываем все тултипы при уходе мыши с карты
		map.on('mouseout', function() {
			map.closePopup();
			map.closeTooltip();
		});

		// Для каждого маркера города отключаем постоянный тултип
		map.on('popupopen', function(e) {
			// Закрываем все остальные тултипы при открытии нового
			map.eachLayer(function(layer) {
				if (layer.closeTooltip && layer !== e.popup) {
					layer.closeTooltip();
				}
			});
		});
...
function openSettlementModal(settlementId) {
    ...
    const rhetoricName = (typeof RHETORIC_NAMES_RU !== 'undefined' && RHETORIC_NAMES_RU[settlement.rhetoric]) ? RHETORIC_NAMES_RU[settlement.rhetoric] : settlement.rhetoric;
...
function zoomToSettlement(settlementId) {
    ...
    // Подсветим маркер, если он есть (можно добавить временное выделение)
    // Здесь можно также показать tooltip, но для простоты просто центрируем
}
