...
function toggleMarkerMode() {
    ...
    const panel = document.getElementById('map-panel-markers') || document.querySelector('.map-panel-markers');
    if (panel) {
        panel.style.display = markerMode ? 'block' : 'none';
    }
    if (btn) {
        btn.classList.toggle('active', markerMode);
        btn.textContent = markerMode ? '📍 Режим добавления (клик)' : '📍 Маркеры';
    }
    if (markerMode) {
...
console.log("✅ 20_markers.js обновлён — каплевидные маркеры, загрузка PNG, правая панель");