...
function getCurrentProvinceRaces() {
    ...
    // Возвращаем прямую ссылку, чтобы изменения сохранялись
    return provincesData[currentProvince].races;
}
...
function getBaseConscriptionLimit() {
	...
	// Бонус к призывному резерву
	let techBonuses = {};
	if (typeof getTechBonuses === 'function') techBonuses = getTechBonuses();
	if (techBonuses.conscriptionBonus) {
		base = Math.floor(base * (1 + techBonuses.conscriptionBonus / 100));
	}
    // Бонус казарм
    let barracksBonus = 0;
    for (let pid of provinces) {
...
function getTaxpayers() {
    ...
    return Math.max(0, totalPop - poor);
}

/**
 * Возвращает суммарный недельный доход со всех провинций фракции
 */
function getWeeklyIncome() {
    ...
    for (let pid of provinces) {
        ...
        let provPop = 0;
        for (let race of prov.races) provPop += getRaceTotal(race);
        const poor = Math.floor(provPop * (peopleState.settings.poorPercent / 100));
        const taxpayers = Math.max(0, provPop - poor);
        totalIncome += Math.floor(taxpayers * peopleState.settings.taxRate);
    }
	// Бонус к налогам
	let techBonuses = {};
	if (typeof getTechBonuses === 'function') techBonuses = getTechBonuses();
	if (techBonuses.taxBonus) {
		totalIncome = Math.floor(totalIncome * (1 + techBonuses.taxBonus / 100));
	}
    return totalIncome;
}

/**
 * ЕДИНАЯ КАЗНА: возвращает общую казну фракции.
 * Использует глобальную функцию из buildings.js, если доступна.
 */
function getCurrentTreasury() {
    if (typeof getFactionTreasury === 'function') {
        return getFactionTreasury();
    }
    // fallback
    let total = 0;
...
/**
 * Устанавливает общую казну фракции.
 */
function setCurrentTreasury(amount) {
    if (typeof setFactionTreasury === 'function') {
        setFactionTreasury(amount);
        return;
    }
    // fallback: сохраняем в текущую провинцию
    if (provincesData[currentProvince]) {
        provincesData[currentProvince].resources.ers = amount;
    }
}
...
function getCurrentTotalArmySize() {
    ...
    for (let army of armies) {
        ...
        // Учитываем очередь рекрутинга
        if (army.recruitmentQueue) {
            for (let q of army.recruitmentQueue) {
                const peoplePerUnit = q.unitTemplate ? q.unitTemplate.countPerUnit : 100;
                total += q.count * peoplePerUnit;
            }
        }
    }
    return total;
            ...
            if (army.recruitmentQueue) {
                for (let q of army.recruitmentQueue) {
                    ...
                    if (template.gender === 'female') {
                        used[race].female += people;
                    } else {
                        used[race].male += people;
                    }
                }
            }
        }
...
function openAddRaceModal() {
    ...
    const predefinedRaces = [
		...
		{ name: "Ледяные эльфы", adultMale: 500, adultFemale: 500, children: 150, elders: 100, birthRate: 2.0, deathRate: 0.1 },
...
function renderRaceTable() {
    ...
    if (halfBreeds.length > 0) {
        ...
        for (let hb of halfBreeds) {
            ...
            // Название расы (редактируемое)
            const cellName = row.insertCell(0);
            cellName.style.padding = '2px 0';
            const nameInput = document.createElement('input');
            nameInput.type = 'text';
            nameInput.value = hb.name;
            nameInput.style.cssText = 'width: 220px; background: #1f1c14; border: 1px solid #b87c4f; color: #f0e6d0; padding: 2px 6px; border-radius: 12px;';
            nameInput.addEventListener('change', () => {
                hb.name = nameInput.value.trim();
                refreshPeopleUI();
                if (typeof refreshRecruitmentLimits === 'function') refreshRecruitmentLimits();
                addGlobalLog(`Полукровка переименована в "${hb.name}".`, 'general');
            });
            cellName.appendChild(nameInput);

            // Численность (кликабельная)
            const cellPop = row.insertCell(1);
            cellPop.style.padding = '2px 0';
            const popSpan = document.createElement('span');
            popSpan.textContent = total.toLocaleString();
            popSpan.style.cursor = 'pointer';
            popSpan.title = 'Нажмите, чтобы редактировать группы';
            popSpan.addEventListener('click', () => {
                const detailDiv = document.createElement('div');
                detailDiv.innerHTML = `
                    <label>Мужчины: <input type="number" value="${hb.adultMale}" class="edit-adult-male" style="width:70px"></label>
                    <label>Женщины: <input type="number" value="${hb.adultFemale}" class="edit-adult-female" style="width:70px"></label>
                    <label>Дети: <input type="number" value="${hb.children}" class="edit-children" style="width:70px"></label>
                    <label>Старики: <input type="number" value="${hb.elders}" class="edit-elders" style="width:70px"></label>
                    <button class="save-groups-btn">Сохранить</button>
                `;
                const existingDetail = row.querySelector('.detail-panel');
                if (existingDetail) existingDetail.remove();
                ...
                detailCell.appendChild(detailDiv);
                detailDiv.querySelector('.save-groups-btn').addEventListener('click', () => {
                    hb.adultMale = parseInt(detailDiv.querySelector('.edit-adult-male').value) || 0;
                    hb.adultFemale = parseInt(detailDiv.querySelector('.edit-adult-female').value) || 0;
                    hb.children = parseInt(detailDiv.querySelector('.edit-children').value) || 0;
                    hb.elders = parseInt(detailDiv.querySelector('.edit-elders').value) || 0;
                    detailCell.remove();
                    refreshPeopleUI();
                    if (typeof refreshRecruitmentLimits === 'function') refreshRecruitmentLimits();
                    addGlobalLog(`Группы расы "${hb.name}" обновлены.`, 'general');
                });
            });
            cellPop.appendChild(popSpan);
            ...
            cellAction.style.padding = '2px 0';
            const delBtn = document.createElement('button');
            delBtn.innerText = '🗑️';
            delBtn.style.background = '#7a3a2a';
            ...
            delBtn.addEventListener('click', () => {
                if (confirm(`Удалить расу "${hb.name}"?`)) {
                    const races = getCurrentProvinceRaces();
                    const idx = races.findIndex(r => r.name === hb.name);
                    if (idx !== -1) {
                        races.splice(idx, 1);
                        refreshPeopleUI();
                        if (typeof refreshRecruitmentLimits === 'function') refreshRecruitmentLimits();
                        addGlobalLog(`Раса "${hb.name}" удалена.`, 'general');
                    }
                }
            });
            cellAction.appendChild(delBtn);
...
const ALLOWED_HALF_BREEDS = [
    ...
    ["Люди","Ледяные эльфы"],
    ["Люди","Вульфины"],
    ["Люди","Лепусиды (высшие)"],
    ["Люди","Лепусиды (карликовые)"],
    ["Гоблины","Дварфы"],
    ["Дварфы","Высшие эльфы"],
    ["Дварфы","Ледяные эльфы"],
    ["Дварфы","Вульфины"],
    ["Высшие эльфы","Ледяные эльфы"],
    ["Высшие эльфы","Вульфины"],
    ["Высшие эльфы","Лепусиды (высшие)"],
    ["Ледяные эльфы","Вульфины"],
    ["Люди","Тайро"],
	["Люди","Ледяные эльфы"],
	["Дварфы","Ледяные эльфы"],
	["Высшие эльфы","Ледяные эльфы"],
	["Вульфины","Ледяные эльфы"],
	["Оку","Ледяные эльфы"]
];
...
function applyDemography() {
    ...
    const birthMultiplier = (peopleState.demography.birthRate || 2) / 2.0;
    const deathMultiplier = (peopleState.demography.deathRate || 1) / 1.0 * 0.1;
    ...
    for (let pid of provinces) {
        ...
        let hasPantheon = false, hasSanctuary = false;
        ...
        let races = prov.races;

        // ----- Очистка дубликатов и запрещённых полукровок -----
        const seen = new Set();
        ...
        for (let race of races) {
            ...
            let births = Math.floor(race.adultFemale * (effectiveBirthRate / 100));
            race.children = Math.max(0, race.children + births);

            // Дополнительные эффекты только для чистокровных
            if (!race.name.startsWith("Полукровка")) {
                // Рождение полукровок (15% от рождений чистой расы)
                if (ALLOWED_HALF_BREEDS.length > 0) {
                    const halfBreedChildren = Math.floor(births * 0.15);
                    if (halfBreedChildren > 0) {
                        const possiblePairs = ALLOWED_HALF_BREEDS.filter(p => p[0] === race.name || p[1] === race.name);
            ...
            // Взросление (5% детей)
            const newAdults = Math.floor(race.children * 0.05);
            race.children = Math.max(0, race.children - newAdults);
            if (race.name === "Вульфины") {
                race.adultMale += Math.floor(newAdults * 0.9);
                race.adultFemale += newAdults - Math.floor(newAdults * 0.9);
            } else {
                race.adultMale += Math.floor(newAdults / 2);
            ...
            if (race.name !== "Высшие эльфы" && effectiveDeathRate > 0) {
                ...
                if (totalAdults > 0) {
                    const adultDeaths = Math.min(remaining, Math.floor(deaths * totalAdults / totalPop));
                    remaining -= adultDeaths;
                    race.adultMale = Math.max(0, race.adultMale - Math.floor(adultDeaths * race.adultMale / totalAdults));
                    race.adultFemale = Math.max(0, race.adultFemale - (adultDeaths - Math.floor(adultDeaths * race.adultMale / totalAdults)));
                }
                race.elders = Math.max(0, race.elders - remaining);
            }

            // Перенаселение лепусидов
            if (race.name.startsWith("Лепусиды") && getRaceTotal(race) > 10000) {
                const excess = getRaceTotal(race) - 10000;
                const reduction = Math.floor(excess * 0.01);
                race.adultMale = Math.max(0, race.adultMale - Math.floor(reduction * 0.5));
                race.adultFemale = Math.max(0, race.adultFemale - Math.ceil(reduction * 0.5));
                race.children = Math.max(0, race.children - Math.floor(reduction * 0.2));
            }

            totalAfter += getRaceTotal(race);
        }

        // Сохраняем очищенный и обновлённый список рас обратно в провинцию
        prov.races = races;
    }

    const change = totalAfter - totalBefore;
    addGlobalLog(`📊 Демография: ${totalBefore.toLocaleString()} → ${totalAfter.toLocaleString()} (изм. ${change >= 0 ? '+' : ''}${change}).`, 'general');
...
function applyTreasuryChange() {
    ...
    const newTreasury = getCurrentTreasury() + amount;
...
function updateTreasuryDisplay() {
    const el = document.getElementById('totalTreasury');
    if (el) el.innerText = Math.floor(window.factionTreasury || 0);
}
...
function applyPeopleTurn() {
    ...
    const newTreasury = getCurrentTreasury() + income;
    setCurrentTreasury(newTreasury);
    addGlobalLog(`💰 Собрано налогов: ${income.toLocaleString()}. Казна: ${newTreasury.toLocaleString()}.`, 'general');
...
/**
 * Пересчитывает максимальное количество одновременных строек
 * на основе количества провинций: база 2 + 2 за каждую провинцию сверх одной.
 */
function recalcMaxConstructionSlots() {
    const provinceCount = Object.keys(provincesData).length;
    const newLimit = 2 + (provinceCount - 1) * 2;
    if (newLimit !== peopleState.maxConstructionSlots) {
        peopleState.maxConstructionSlots = newLimit;
        console.log(`🔧 Лимит строек обновлён: ${newLimit} (провинций: ${provinceCount})`);
    }
    // Обновляем отображение, если есть элемент
    const info = document.getElementById('constructionInfo');
    if (info) {
        info.innerText = `🏗️ Строек: ${peopleState.activeConstructionCount || 0} / ${peopleState.maxConstructionSlots}`;
    }
}
...
// ========== 13. ЭКСПОРТ ФУНКЦИЙ В ГЛОБАЛЬНУЮ ОБЛАСТЬ ==========
...
window.setCurrentTreasury = setCurrentTreasury;
...
/**
 * Вычитает убитых солдат из общего населения фракции равномерно по провинциям
 * @param {string} raceName - название расы
 * @param {string} gender - 'male' или 'female'
 * @param {number} amount - количество убитых
 */
function deductPopulation(raceName, gender, amount) {
    const provinces = (typeof getCurrentFactionProvinces === 'function') ? getCurrentFactionProvinces() : [];
    if (provinces.length === 0) return;
    
    const perProvince = Math.floor(amount / provinces.length);
    let remainder = amount - perProvince * provinces.length;

    for (let pid of provinces) {
        const prov = provincesData[pid];
        if (!prov || !prov.races) continue;
        const race = prov.races.find(r => r.name === raceName);
        if (!race) continue;

        let deduct = perProvince;
        if (remainder > 0) {
            deduct++;
            remainder--;
        }
        if (gender === 'female') {
            race.adultFemale = Math.max(0, (race.adultFemale || 0) - deduct);
        } else {
            race.adultMale = Math.max(0, (race.adultMale || 0) - deduct);
        }
    }
...
console.log("✅ population.js загружен — версия 6.1 (единая казна, исправленная демография)");