...
function payArmyUpkeep() { return true; }
...
function getUnitsForCurrentFaction() {
    ...
    for (let unitKey of pool.units) {
        if (db[unitKey]) {
            ...
            // Пропускаем юнитов, привязанных к другой фракции
    ...
    for (let [key, unit] of Object.entries(mercs)) {
        // Для наёмников тоже проверяем, если указано поле faction
        if (unit.faction && unit.faction !== window.currentFaction) continue;
        units.push({ key, ...unit });
    }
    return units;
...
function checkUnitLimit(unitKey) {
    ...
    const maxAllowed = unitDef.maxCount;
