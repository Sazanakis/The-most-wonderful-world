...
function showTreasuryBreakdown() {
    ...
    for (let pid of provinces) {
        ...
        baseTaxIncome += Math.floor(taxpayers * (peopleState?.settings?.taxRate || 1));
    }
    const actualTaxIncome = (typeof getWeeklyIncome === 'function') ? getWeeklyIncome() : baseTaxIncome;

    // --- КОРРУПЦИЯ ---
    const corruptionPercent = (typeof peopleState !== 'undefined' && peopleState.corruption) 
        ? peopleState.corruption.currentPercent 
        : 0;
    const corruptionLoss = Math.floor(actualTaxIncome * (corruptionPercent / 100));
    const finalTaxIncome = actualTaxIncome - corruptionLoss;

    // --- ЧИНЫ КАНЦЕЛЯРИИ (7%) ---
    const chancelleryCost = Math.floor(finalTaxIncome * 0.07);

    // --- ТОРГОВЛЯ ---
    const tradeBonusPercent = (typeof getTradeBonusPercent === 'function') ? getTradeBonusPercent() : 0;
    let exportBase = 0;
    let importBase = 0;
    ...
    const exportWithBonus = Math.floor(exportBase * (1 + tradeBonusPercent / 100));
    const totalTradeIncome = exportWithBonus - importBase;
    ...
    if (typeof researchData !== 'undefined' && researchData.completedTechs) {
        for (let techId of researchData.completedTechs) {
            ...
            if (tech.effects.taxBonus) {
                techTaxBonus += tech.effects.taxBonus;
                techTaxDetails.push({ name: tech.name, bonus: tech.effects.taxBonus });
            }
        }
    }
    ...
    if (typeof researchData !== 'undefined' && researchData.completedTechs) {
        for (let techId of researchData.completedTechs) {
            ...
            if (tech.effects.tradeBonus) {
                techTradeBonus += tech.effects.tradeBonus;
                techTradeDetails.push({ name: tech.name, bonus: tech.effects.tradeBonus });
            }
        }
    }

    let buildingTradeBonus = 0;
    let buildingTradeDetails = [];
    for (let pid of provinces) {
        const prov = provincesData[pid];
        if (!prov) continue;
        for (let s of prov.settlements) {
            ...
            for (let b of s.buildings) {
                if (!b.completed) continue;
                if (b.special === "tradeBonus15") {
                    buildingTradeBonus += 15;
                    buildingTradeDetails.push(`${b.name} (+15%)`);
                } else if (b.special === "tradeBonus30") {
                    buildingTradeBonus += 30;
                    buildingTradeDetails.push(`${b.name} (+30%)`);
                }
            }
        }
    }
    ...
    <div style="background:#1f1c14; border:2px solid #b87c4f; border-radius:20px; padding:20px; max-width:600px; width:90%; color:#e6ddb3; text-align:left;">
        <h3 style="color:#ffd966; margin-top:0; text-align:center;">💰 Детали казны</h3>
        <table style="width:100%; border-collapse:collapse;">
            <tr>
                <td>🏘️ Налоги (после коррупции)</td>
                <td style="text-align:right;">+${finalTaxIncome.toLocaleString()} эрсов</td>
            </tr>`;

    if (techTaxDetails.length > 0) {
        html += `
            <tr><td colspan="2">
                <details style="margin-left:20px;">
                    <summary style="cursor:pointer; color:#8bc34a;">🎓 Технологии (налоги) +${techTaxBonus}%</summary>
                    ${techTaxDetails.map(t => `<div>• ${t.name}: +${t.bonus}%</div>`).join('')}
                    <div style="margin-top:4px;">База налогов: ${baseTaxIncome.toLocaleString()} → с бонусом: ${actualTaxIncome.toLocaleString()}</div>
                </details>
            </td></tr>`;
    }
    ...
    html += `
            <tr>
                <td>🤌 Коррупция (${corruptionPercent}%)</td>
                <td style="text-align:right;">−${corruptionLoss.toLocaleString()} эрсов</td>
            </tr>
            <tr>
                <td>📜 Чины Канцелярии (7%)</td>
                <td style="text-align:right;">−${chancelleryCost.toLocaleString()} эрсов</td>
            </tr>

            <!-- Торговля: развёрнуто -->
            <tr>
                <td>📤 Экспорт (база)</td>
                <td style="text-align:right;">+${exportBase.toLocaleString()} эрсов</td>
            </tr>`;

    if (tradeBonusPercent > 0) {
        html += `
            <tr>
                <td>📈 Бонус рынков и технологий</td>
                <td style="text-align:right; color:#8bc34a;">+${(exportWithBonus - exportBase).toLocaleString()} эрсов (${tradeBonusPercent}%)</td>
            </tr>`;
    }

    html += `
            <tr>
                <td>📥 Импорт (расходы)</td>
                <td style="text-align:right; color:#ff6b6b;">−${importBase.toLocaleString()} эрсов</td>
            </tr>`;

    if (techTradeDetails.length > 0 || buildingTradeDetails.length > 0) {
        html += `
            <tr><td colspan="2">
                <details style="margin-left:20px;">
                    <summary style="cursor:pointer; color:#8bc34a;">⚙️ Состав бонуса (${tradeBonusPercent}%)</summary>`;
        if (techTradeDetails.length > 0) {
            html += `<div><strong>Технологии:</strong></div>`;
            techTradeDetails.forEach(t => html += `<div>• ${t.name}: +${t.bonus}%</div>`);
        }
        if (buildingTradeDetails.length > 0) {
            html += `<div style="margin-top:4px;"><strong>Постройки:</strong></div>`;
            buildingTradeDetails.forEach(d => html += `<div>• ${d}</div>`);
        }
        html += `</details></td></tr>`;
    }
    ...
    html += `
            ...
            <tr>
                <td>👨‍🔬 Зарплаты исследователей</td>
                <td style="text-align:right;">−${researcherSalaries.toLocaleString()} эрсов</td>
            </tr>
            <tr style="border-top:2px solid #ffd966;">
                <td><strong>Чистое изменение за ход</strong></td>
                <td style="text-align:right; font-weight:bold; color:${netChange >= 0 ? '#8bc34a' : '#ff6b6b'};">${netChange >= 0 ? '+' : ''}${netChange.toLocaleString()} эрсов</td>
            </tr>
            <tr>
                <td><strong>Текущая казна</strong></td>
                <td style="text-align:right; font-weight:bold;">${currentTreasury.toLocaleString()} эрсов</td>
            </tr>
        </table>
    ...
    const oldModal = document.getElementById('treasuryBreakdownModal');
    if (oldModal) oldModal.remove();

    const modal = document.createElement('div');
    modal.id = 'treasuryBreakdownModal';
