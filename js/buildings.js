...
function getVassalForSettlement(settlementId) {
    ...
    // Если не нашли, смотрим в SETTLEMENTS_DB (исходное состояние)
...
function collectResources() {
    // Один раз получаем бонусы от технологий (общие для всей фракции)
            ...
            for (let building of settlement.buildings) {
				...
				if (building.completed && building.special === "titanForge") {
					if (prov.resources.wood >= 5) {
						...
						addBuildingsLog(`❌ Кузница Титанов в "${settlement.name}" не работает: недостаточно древесины (нужно 5).`);
					}
					continue;   // пропускаем стандартный income
				}
                if (building.completed && building.income) {
                    ...
                    // --- Применяем бонус от технологий к железу ---
        ...
        for (let settlement of prov.settlements) {
            if (settlement.completed === false && settlement.remainingTurns > 0) {
                ...
                if (settlement.remainingTurns === 0) {
                    ...
                    addBuildingsLog(`✅ Завершено строительство поселения "${settlement.name}" в провинции "${provinceName}".`);
                }
            }
        }

        // --- 2. Обычные постройки внутри уже готовых поселений ---
        for (let settlement of prov.settlements) {
            if (settlement.captured) continue;
            if (!settlement.completed) continue; // ещё строящееся поселение
            ...
            for (let building of settlement.buildings) {
                if (!building.completed && building.remainingTurns > 0) {
                    ...
                    if (building.remainingTurns === 0) {
                        ...
                        addBuildingsLog(`🏗️ В провинции "${provinceName}" завершено строительство "${building.name}" в "${settlement.name}".`);
                    }
                }
            }
        }
