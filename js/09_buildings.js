...
// ВЕРСИЯ 4.0 — РЕФАКТОРИНГ, ЗАЩИТА ОТ ОТСУТСТВИЯ ГЛОБАЛЬНЫХ ДАННЫХ
// ============================================================================
...
function showBuildingSelector(settlementId) {
    ...
    modal.style.cssText = 'position:fixed; top:50%; left:50%; transform:translate(-50%,-50%); background:#1f1c14; border:2px solid #b87c4f; border-radius:24px; padding:20px; z-index:2000; display:flex; flex-direction:column; gap:8px; max-width:500px; width:90%; max-height:80vh; overflow-y:auto;';
    modal.innerHTML = `<h3>Выберите постройку для ${escapeHtml(settlement.name)}</h3>`;
...
function startBuilding(settlementId, buildingName, isUpgrade = false, baseBuilding = null) {
    ...
    let buildingDef, cost, buildTime, level = 1, name = buildingName;
    if (isUpgrade && baseBuilding) {
        const upgradeDef = (typeof buildingsCatalog !== 'undefined' && buildingsCatalog[baseBuilding.baseName]) ? buildingsCatalog[baseBuilding.baseName].upgrade : null;
        ...
        level = 2;
        name = upgradeDef.nameLevel2 || `${baseBuilding.name} (улучшенная)`;
        buildingDef = { ...baseBuilding, level: 2, name: name, income: upgradeDef.income, special: upgradeDef.special2 || baseBuilding.special };
    } else {
        buildingDef = (typeof buildingsCatalog !== 'undefined') ? buildingsCatalog[buildingName] : null;
    ...
    if (isUpgrade && baseBuilding) {
        ...
        if (idx !== -1) {
            settlement.buildings[idx] = {
                ...
                level: level,
                baseName: baseBuilding.baseName,
                isUpgrade: true,
                income: buildingDef.income,
                special: buildingDef.special
            };
        }
    } else {
    ...
    addBuildingsLog(`${isUpgrade ? 'Улучшение' : 'Строительство'} "${name}" начато в "${settlement.name}". Затрачено ресурсов.`);
    saveAllData();
    refreshBuildingsUI();
            ...
            for (let building of settlement.buildings) {
                if (!building.completed && building.remainingTurns > 0) {
                    ...
                    if (building.remainingTurns === 0) {
                        ...
                        addBuildingsLog(`🏗️ В провинции "${(typeof PROVINCE_NAMES !== 'undefined' && PROVINCE_NAMES[pid]) ? PROVINCE_NAMES[pid] : pid}" завершено строительство "${building.name}" в "${settlement.name}".`);
                    }
                }
            }
        }
...
function collectResources() {
    for (let pid in provincesData) {
        ...
        for (let settlement of prov.settlements) {
            for (let building of settlement.buildings) {
                if (building.completed && building.income) {
                    ...
                    if (building.special === "ironBonus") ironInc = 0;
        ...
        for (let settlement of prov.settlements) {
            for (let building of settlement.buildings) {
                if (building.completed && building.name === "Кузница" && building.level === 1) forgeBonus = 1.5;
                if (building.completed && building.name === "Кузница (улучшенная)") forgeBonus = 2.0;
            }
        }
        prov.resources.iron = Math.floor(prov.resources.iron * forgeBonus);

        if (dwarfBonus > 1.0 && (stoneIncome > 0 || ironIncome > 0)) {
            addGlobalLog(`⛏️ Дварфы увеличили добычу камня и железа в ${(typeof PROVINCE_NAMES !== 'undefined' && PROVINCE_NAMES[pid]) ? PROVINCE_NAMES[pid] : pid} на ${Math.round((dwarfBonus-1)*100)}%.`, 'general');
        }
    }
}
...
function exchangeGold() {
    ...
    for (let settlement of data.settlements) {
        for (let b of settlement.buildings) {
            if (b.completed && b.name === "Монетный двор") mintRate = 120;
            if (b.completed && b.name === "Монетный двор (улучшенный)") mintRate = 140;
        }
    }
    if (data.resources.gold <= 0) { addBuildingsLog(`Нет золота для обмена.`); return; }
    const gained = data.resources.gold * mintRate;
    data.resources.ers += gained;
    addBuildingsLog(`💰 Обменяно ${data.resources.gold} золота на ${gained} эрсов (курс ${mintRate}).`);
...
console.log("✅ 09_buildings.js загружен — версия 4.0 (без торговых функций)");