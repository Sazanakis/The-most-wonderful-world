...
function removeSettlementFromVassal(settlementId) {
    let found = false;
    // Ищем поселение во всех провинциях
    for (let pid in provincesData) {
        const prov = provincesData[pid];
        if (!prov || !prov.settlements) continue;
        const settlement = prov.settlements.find(s => s.id === settlementId);
        if (settlement) {
            // Принудительно удаляем вассала
            settlement.vassalHouse = null;
            found = true;
            break;
        }
    }
    // Если не нашли в provincesData, возможно, поселение ещё не инициализировано,
    // тогда сбрасываем в SETTLEMENTS_DB (на будущее)
    if (!found && typeof SETTLEMENTS_DB !== 'undefined') {
        const dbSettlement = SETTLEMENTS_DB[settlementId];
        if (dbSettlement && dbSettlement.vassalHouse) {
            dbSettlement.vassalHouse = null;
            found = true;
        }
    }
    ...
    if (found) {
        ...
        refreshBuildingsUI();
        if (typeof addGlobalLog === 'function') {
            addGlobalLog(`🏘️ Поселение успешно отнято у вассала.`, 'council');
        }
        // Перерисовываем модальное окно вассала, если оно открыто
        const currentModalHouseId = document.getElementById('vassalModal')?.getAttribute('data-house-id');
        if (currentModalHouseId && typeof openVassalModal === 'function') {
            openVassalModal(currentModalHouseId);
        }
        return true;
    }
    return false;
