...
// ВЕРСИЯ 8.0 — ЯДРО (ЛОГИКА БЕЗ UI)
// Содержит все функции управления армиями, найма, очередей, сохранения,
// но без прямого DOM-рендера. Функции расчёта лимитов призыва берутся из 08_population.js
// ============================================================================

// ========== 1. ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (ЕСЛИ НЕ ОПРЕДЕЛЕНЫ В 08_population.js) ==========
...
// Функции лимитов призыва – перенаправляем на 08_population.js, если они там есть
...
function createNewArmy(name = null) {
    ...
    modal.querySelector('#confirmCreateArmyBtn').onclick = () => {
        ...
        armies.push(newArmy);
        saveArmyData();
        if (typeof renderArmy === 'function') renderArmy();   // ← ГЛАВНОЕ: обновляем интерфейс
...
// ========== 9. ФИЛЬТРАЦИЯ ЮНИТОВ (ПЕРЕМЕННЫЕ ИЗ 02_variables.js) ==========
...
console.log("✅ 07_army_core.js загружен — ядро армий (без UI)");