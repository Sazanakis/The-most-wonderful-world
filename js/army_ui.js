    ...
    for (let army of filteredArmies) {
                ...
                unitsHtml += `
					<div style="display:flex; flex-direction:column; align-items:center; background:#1f1c14; padding:6px; border-radius:10px; width:110px; text-align:center; position:relative;">
						...
						<div style="display:flex; gap:4px; justify-content:center; margin-top:4px;">
							<button onclick="window.removeUnitFromArmy('${army.id}', '${unit.id}')" style="background:#7a2a2a; padding:2px 6px; font-size:0.6rem;">✖</button>
							<button onclick="openUnitManualEdit('${army.id}', '${unit.id}')" style="background:#b8860b; padding:2px 6px; font-size:0.6rem;">🗡️</button>
						</div>
					</div>`;
            }
            unitsHtml += '</div>';
        ...
        card.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center;">
				...
				<div>
					<button onclick="openEditArmyModal('${army.id}')" style="background:#3a5a2a; padding:4px 8px; font-size:0.7rem;">✏️</button>
					<button onclick="window.lastSelectedArmyId='${army.id}'; alert('Армия выбрана для найма')" style="background:#3a5a2a; padding:4px 8px; font-size:0.7rem;">🎯 Выбрать</button>
					<button onclick="openBattleModal('${army.id}')" style="background:#6b3a1a; padding:4px 8px; font-size:0.7rem;">⚔️</button>
					<button onclick="if(confirm('Снять армию с гарнизона?')) { window.updateArmyInfo('${army.id}', {garrison: null}); }" style="background:#b8860b; padding:4px 8px; font-size:0.7rem;">🏕️</button>
					<button onclick="window.deleteArmy('${army.id}')" style="background:#7a2a2a; padding:4px 8px; font-size:0.7rem;">🗑️</button>
				</div>
            </div>
            ${unitsHtml}
            ${queueHtml}
...
function openUnitDetailModal(unit) {
    ...
    // Функция для красивого вывода числа с бонусом
    function formatStat(baseValue, bonusKey, showSign = false) {
        const bonus = techBonuses[bonusKey] || 0;
        const total = baseValue + bonus;
        const bonusText = bonus !== 0
            ? ` (база: ${baseValue} ${bonus >= 0 ? '+' : ''}${bonus} от технологий)`
            : '';
        return {
            display: total,
            title: bonusText,
    ...
    // --- Характеристики с возможными бонусами ---

    // Атака ближняя
    const meleeBase = unit.strengthMelee || unit.strength || 0;
    const meleeInfo = formatStat(meleeBase, 'meleeAttackBonus');

    // Атака дальняя
    const rangedBase = unit.strengthRanged || 0;
    const rangedInfo = formatStat(rangedBase, 'rangedAttackBonus');

    // Защита
    const defenseBase = unit.defense;
    const defenseInfo = formatStat(defenseBase, 'infantryDefenseBonus');

    // Мораль
    const moraleBase = unit.morale;
    const moraleInfo = formatStat(moraleBase, 'globalMoraleBonus');

    // Содержание
    const upkeepBase = unit.upkeep || 0;
    const upkeepInfo = formatStat(upkeepBase, 'upkeepReduction', true);

    // Стоимость найма
    const hireCostBase = unit.hireCost || 0;
    const hireCostInfo = formatStat(hireCostBase, 'hireCostReduction', true);

    // Время найма
    const hireTimeBase = unit.hireTime || 1;
    const hireTimeInfo = formatStat(hireTimeBase, 'hireTimeReduction', true);

    const costStr = unit.hireCost === 0 ? 'бесплатно' : `${unit.hireCost} эрсов`;
    ...
    modal.innerHTML = `
        ...
        ">
            ...
            <div style="flex: 1; display: flex; flex-direction: column; gap: 8px;">
                ...
                <div><strong>⚖️ Содержание:</strong> ${upkeepInfo.display} ${upkeepInfo.title}</div>
                <div><strong>⚔️ Атака ближняя:</strong> ${meleeInfo.display} ${meleeInfo.title}</div>
                <div><strong>🏹 Атака дальняя:</strong> ${rangedInfo.display} ${rangedInfo.title}</div>
                <div><strong>🛡️ Защита:</strong> ${defenseInfo.display} ${defenseInfo.title}</div>
                <div><strong>❤️ Мораль:</strong> ${moraleInfo.display} ${moraleInfo.title}</div>
                <div><strong>⏱️ Время найма:</strong> ${hireTimeInfo.display} ход(ов) ${hireTimeInfo.title}</div>
...
function openUnitManualEdit(armyId, unitId) {
    ...
    document.getElementById('reinforceBtn').addEventListener('click', () => {
        ...
        // Пересчитываем доступный резерв (он мог измениться после предыдущих действий)
        ...
        // Пополняем отряд (резерв тратится автоматически, так как увеличивается used)
        unit.count += deficit;
        addGlobalLog(`📥 Отряд "${unit.name}" пополнен на ${deficit} чел. из резерва.`, 'army');
        finishAndClose();
    });
