...
function setupMapUI() {
    ...
    setupCollapsible('leftHeader', 'leftContent');
    setupCollapsible('citiesHeader', 'citiesContent');
    setupCollapsible('routesHeader', 'routesContent');
    ...
    if (markerModeBtn && markersPanel) {
        markerModeBtn.addEventListener('click', function() {
            const isVisible = markersPanel.style.display !== 'none';
            markersPanel.style.display = isVisible ? 'none' : 'block';
            markerModeBtn.classList.toggle('active', !isVisible);
            if (!isVisible && typeof window.toggleMarkerMode === 'function') {
    ...
    function setActiveTab(activeBtn) {
        ...
        // Показываем нужную панель и вызываем соответствующие маски
        if (activeBtn === rhetoricBtn && rhetoricPanel) {
            rhetoricPanel.style.display = 'flex';
            if (typeof showRhetoricMasks === 'function') {
                showRhetoricMasks('all');
                console.log('🎭 Маски риторики включены');
            }
            ...
            if (typeof showHoldingsMask === 'function') {
                showHoldingsMask();   // без аргументов
                console.log('🏰 Маска владений включена');
            }
            ...
            if (typeof showVassalsMask === 'function') {
                showVassalsMask();    // без аргументов
                console.log('⚔️ Маска вассалов включена');
            }
            if (typeof updateVassalChecklist === 'function') updateVassalChecklist();
        } else if (activeBtn === routesBtn) {
            // Для маршрутов — очищаем маски и перерисовываем маршруты
            if (typeof clearMasks === 'function') clearMasks();
            if (typeof redrawRoutes === 'function') redrawRoutes();
            console.log('🗺️ Маршруты');
        }
